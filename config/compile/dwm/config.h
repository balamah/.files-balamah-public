...
#define TERMCLASS "tabbed"

#define SCRIPT "~/.config/scripts/"
#define DMSCRIPT "pgrep -x dmenu || ~/.config/scripts/dmenu/"
#define WIDGET "~/.config/scripts/widgets/launch-widget ~/.config/scripts/widgets/start/"
...
#if BAR_PANGO_PATCH
static const char font[]                 = "JetBrainsMonoNL NFM 10";
#else
static const char *fonts[]               = { "JetBrainsMonoNL NFM:size=10" };
#endif // BAR_PANGO_PATCH
static const char dmenufont[]            = "JetBrainsMonoNL NFM:size=10";
 ...
 *    // class      instance  title  wintype  tags mask  isfloating  monitor  noswallow
...
static const Rule rules[] = {
	...
	RULE(.class = NULL, .instance = NULL, .title = "Event tester", .noswallow = 1)
	...
	RULE(.class = "obs", .tags = 1 << 7, .monitor = 1)
	RULE(.class = "DBeaver", .tags = 1 << 7, .monitor = 1)
	RULE(.class = "Soundux", .tags = 1 << 7, .monitor = 1)

	RULE(.class = "TelegramDesktop", .tags = 1 << 2)
	RULE(.class = "ViberPC", .tags = 1 << 2, .monitor = 0)
	RULE(.class = "discord", .tags = 1 << 3, .monitor = 0)

	RULE(.class = "Code", .tags = 1 << 5, .monitor = 0)
	RULE(.class = "jetbrains-pycharm-ce", .tags = 1 << 5, .monitor = 0)

	RULE(.class = "Editors", .tags = 1 << 7)
	RULE(.class = "Minecraft Launcher", .tags = 1 << 7, .monitor = 0)
	RULE(.class = "VirtualBox Manager", .tags = 1 << 7)
	RULE(.class = "openrgb", .tags = 1 << 7, .monitor = 0)
	RULE(.class = "steam", .tags = 1 << 7, .monitor = 0)
	RULE(.class = "Pinta", .tags = 1 << 7)
	RULE(.class = "kdenlive", .tags = 1 << 7, .monitor = 0)

	RULE(.class = "zoom", .tags = 1 << 8, .monitor = 0)

	/* sxw widgets */
	RULE(.class = "widget", .isfloating = 1, .noswallow = 1)
...
static const char *tabbedstcmd[] = { "tabbed", "-c", "-r", "2", "st", "-w", "''", NULL };
...
static const Key keys[] = {
	...
	{ MODKEY|ShiftMask,             XK_Return,     spawn,                  {.v = tabbedstcmd } },
	#if RIODRAW_PATCH
	...
	#endif // ALT_TAB_PATCH
	#if SHIFTTAG_PATCH
	{ MODKEY|ShiftMask,             XK_bracketleft,       shifttag,               { .i = -2 } }, // note keybinding conflict with focusadjacenttag tagtoleft
	...
	{ MODKEY|ShiftMask,             XK_F6,         xrdb,                   {.v = NULL } },
	#endif // XRDB_PATCH
	// layouts
	...
	/* custom keybindings */
	/* ATTENTION: be careful with patches. */
	/* You can see xorg keys here: /usr/include/X11/keysymdef.h */
	{ MODKEY,						XK_F6,		xrdb,		{.v = NULL } },
	{ MODKEY|ShiftMask,             XK_q,       spawn,      SHCMD(DMSCRIPT "dwm/logout") },

	// dumb scripts shortcuts
	{ MODKEY|ControlMask|ShiftMask,		XK_q,		spawn,		SHCMD(TERMINAL " -e ~/.config/scripts/connection-test") },
	{ MODKEY|ControlMask|ShiftMask,		XK_w,		spawn,		SHCMD(TERMINAL " -e ~/.config/scripts/webcam-launch") },
	/* { MODKEY|ShiftMask,					XK_w,		spawn,		SHCMD(SCRIPT "additional/wal-telegram/wal-telegram --wal -r") }, */
	{ MODKEY|ControlMask|ShiftMask,		XK_e,		spawn,		SHCMD(SCRIPT "startup/30-wallpaper-set") },
	{ MODKEY,							XK_F5,		spawn,		SHCMD(SCRIPT "wal-sync") },
	{ MODKEY,							XK_p,		spawn,		SHCMD(SCRIPT "color-picker") },
	{ MODKEY|ControlMask|ShiftMask,     XK_r,		spawn,		SHCMD(SCRIPT "kill-programs") },
	{ MODKEY|ShiftMask,					XK_p,		spawn,		SHCMD(SCRIPT "update/weather") },
	{ MODKEY|ControlMask|ShiftMask,		XK_l,		spawn,		SHCMD("slock") },
	{ MODKEY|ControlMask|ShiftMask,     XK_t,		spawn,		SHCMD("killall -9 spotify") },
	{ MODKEY|ControlMask|ShiftMask,     XK_x,		spawn,		SHCMD("xkill") },

	// dmenu scripts
	#if RESTARTSIG_PATCH
	{ MODKEY|ControlMask|ShiftMask,     XK_k,				spawn,		SHCMD(DMSCRIPT "dwm/restart") },
    #endif

	// reader mode
	{ MODKEY,						XK_F7,	spawn,	SHCMD("gummy -t 5000") },
	{ MODKEY,						XK_F8,	spawn,	SHCMD("gummy -t 6500") },
};
...
static const Button buttons[] = {
	...
	#if BAR_STATUSBUTTON_PATCH
	{ ClkButton,            0,                   Button1,        spawn,          SHCMD(WIDGET "dashboard") },
	#endif // BAR_STATUSBUTTON_PATCH
