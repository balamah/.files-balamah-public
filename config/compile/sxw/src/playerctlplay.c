...
redraw(int action)
{
    ...
    /*
	 * the +10 is added because sometimes a small border is left of
	 * the bottom and right part of the button for some reason
	 */
    ...
    char state_string[8];
    sh("playerctl status --format '{{ status }}'", state_string, 8);

    /* state = atoi(state_string); */
	state = getStatus(state_string);
    ...
    if(state && action) {
		char buffer[16];
		sh("~/.config/scripts/app-audio-control pause", buffer, 16);
		state = 0;
    } else if(!state && action){
		char buffer[16];
		sh("~/.config/scripts/app-audio-control play", buffer, 16);
		state = 1;
    }
...
main(int argc, char** argv)
{
    ...
    class_hint->res_name = "playerctlplay";
