...
(defvar chemacs-profiles-path
  (let* ((emacs-configs-path "~/.config/emacs/")
		 (profiles (concat emacs-configs-path "profiles.el"))
		 (local-profiles (concat emacs-configs-path "profiles.local.el")))
	(if (not (file-exists-p local-profiles))
		profiles
	  local-profiles)))

(defvar chemacs-default-profile-path "~/.config/emacs/default.el")
